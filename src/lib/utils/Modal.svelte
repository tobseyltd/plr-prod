...
<script>
	 ...
	 */
	const baseIsTabbable = (
		/** @type {{ tabIndex: number; hidden: any; disabled: any; style: { display: string; }; type: string; offsetWidth: any; offsetHeight: any; getClientRects: () => { (): any; new (): any; length: any; }; }} */ node
	) =>
		node.tabIndex >= 0 &&
		!node.hidden &&
		!node.disabled &&
	...
	const isFunction = (/** @type {{ constructor: any; call: any; apply: any; }} */ f) =>
	...
	const handleKeydown = (
		/** @type {{ key: string; preventDefault: () => void; shiftKey: any; }} */ event
	) => {
	...
	const handleOuterMouseup = (
		/** @type {{ target: any; preventDefault: () => void; }} */ event
	) => {
